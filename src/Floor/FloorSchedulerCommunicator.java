...
public class FloorSchedulerCommunicator {
	 ...
	 * @param floor
	 */
	public FloorSchedulerCommunicator(Floor floor) {
		this.floor = floor;
		try {
	...
	public void sendElevatorRequest(RequestElevatorEvent job) {
		...
		try {
			sendElevatorRequestPacket = new DatagramPacket(msg, msg.length, InetAddress.getLocalHost(), 420);
		} 
	...
	public void requestElevatorInfo() {
		...
		try {
			receiveElevatorInfoPacket = new DatagramPacket(msg, msg.length, InetAddress.getLocalHost(), 420);
		} catch (UnknownHostException e) {
			e.printStackTrace();
			System.exit(1);
		}
		...
		try {
			floor.addElevatorInfo(new ElevatorInfo(reply));
		} catch (Exception e) {
